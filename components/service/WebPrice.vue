...
const onePagePrice = [
  {
    ...
    description: '使用現成的 HTML/CSS/JS 模板，依照客戶品牌簡單修改內容與顏色，不含設計稿、不含後台(後台另報價)。',
  ...
  },
  {
    ...
    description: '依客戶需求挑選 WordPress 主題，協助安裝、調整內容、設置網站後台與 SEO 基礎設定。',
...
const psdToHtmlPrice = [
  {
    title: '基本靜態網頁切版（HTML/CSS/JS）',
    subtitles: [
      {
        subtitle: '價格範圍',
        items: [
          '簡單 Landing Page：NT$3,000 ~ $15,000元',
          '中型企業形象網站：NT$15,000 ~ $50,000元',
          '複雜互動式頁面（如動畫效果）：NT$50,000元起',
        ],
      },
      {
        subtitle: '適用情境',
        items: ['無後端功能、純靜態內容展示', '例如：公司官網、活動頁、產品介紹頁'],
      },
    ],
  },
  {
    ...
    subtitles: [
      {
        subtitle: '價格範圍',
        items: [
          '簡單 SPA（單頁應用）：NT$20,000元起',
          '複雜後台管理介面：NT$80,000元起',
          '整合 API 或第三方服務：額外加 NT$10,000元起',
        ],
      },
      {
        subtitle: '適用情境',
        items: ['需要前端框架（如 Vue、React）的動態功能', '例如：會員系統、即時數據儀表板、電商前台'],
      },
    ],
  },
]
  ...
  <div class="px-6 pt-24 md:pt-32 pb-24 md:px-20 md:space-y-20 space-y-10 relative overflow-x-clip">
    ...
    <div class="space-y-12 max-w-7xl mx-auto">
      ...
      >
        ...
        <p class="text-[#8782FF] text-[16px] md:text-[20px] font-bold mb-10">PRICE LIST</p>
        <div
          v-for="(service, index) in multiPagePrice"
          :key="index"
          class="flex flex-col md:flex-row justify-between items-start md:items-center pb-8"
          :class="{ 'border-b ': index !== multiPagePrice.length - 1 }"
        >
          <div class="space-y-2">
            <div class="text-xl font-medium mb-2">{{ service.title }}</div>
            <p class="text-sm text-gray-600 font-semibold">
    ...
    <div class="space-y-12 max-w-7xl mx-auto">
      ...
      >
        ...
        <p class="text-[#8782FF] text-[16px] md:text-[20px] font-bold mb-10">PRICE LIST</p>
        <div
          v-for="(service, index) in webDesignPrice"
          :key="index"
          class="flex flex-col md:flex-row justify-between items-start md:items-center pb-8"
    ...
    <div class="space-y-12 max-w-7xl mx-auto">
      ...
      >
        ...
        <p class="text-[#8782FF] text-[16px] md:text-[20px] font-bold mb-10">PRICE LIST</p>
        <div class="space-y-2 grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-8 pb-12">
          <div v-for="(service, index) in psdToHtmlPrice" :key="index" class="flex items-start space-x-4">
            <div>
              <div class="text-xl font-medium mb-4">{{ service.title }}</div>
              <ul class="list-decimal pl-6 space-y-4 text-gray-600">
                <li v-for="(section, sectionIndex) in service.subtitles" :key="sectionIndex" class="font-semibold">
                  {{ section.subtitle }}
                  <ul class="list-disc pl-6 space-y-2 font-normal mt-2">
