...
const servicesPrice = computed(
  () => tm('mktg.priceServices') as { title: string; highlight: string; description: string; price: string; unit: string }[],
)
</script>
...
<template>
  <div class="relative px-6 py-24 md:py-32 md:px-20 overflow-hidden">
    ...
    <div class="relative z-10 mb-10 md:mb-20">
      ...
      <div class="text-[32px] md:text-[42px] font-bold leading-tight text-center text-[#2D2D2D]">{{ t('mktg.priceTitle') }}</div>
      ...
      <h1 class="text-gray-600 text-[16px] md:text-[20px] text-center md:mb-4">
        {{ t('mktg.intro1') }}<br />{{ t('mktg.intro2') }}
      </h1>
    </div>
      ...
      <div class="mt-16 p-8 bg-white/40 backdrop-blur-md rounded-[2rem] border border-[#8782ff20] shadow-sm">
        <div class="flex items-start">
          ...
          <p class="text-gray-500 text-sm leading-7">
            <span class="text-[#8782FF] font-bold block mb-1">{{ t('mktg.note') }}</span>
            {{ t('mktg.noteText') }}
          </p>
        </div>
      </div>
